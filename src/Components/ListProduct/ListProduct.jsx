...
export const ListProduct = () => {
  ...
  const fetchInfo = async () => {
    const res = await fetch('https://tienda-backend-au3t.onrender.com/allproducts');
  ...
  const updatedProducts = data.map(product => ({
    ...product,
    image: product.image ? `https://tienda-backend-au3t.onrender.com/images/${product.image}?t=${new Date().getTime()}` : null
  }));
  ...
  const remove_product = async (id) => {
    if (window.confirm("Are you sure you want to delete this product?")) {
      await fetch('https://tienda-backend-au3t.onrender.com/removeproduct', {
        method: 'POST',
        headers: {
  ...
  const updateProduct = async () => {
    ...
    const response = await fetch('https://tienda-backend-au3t.onrender.com/editproduct', {
      method: 'POST',
